...
class vPDOZip {
     ...
     *
     * @param vPDO &$xpdo A reference to an xPDO instance.
     ...
     */
    public function __construct(vPDO &$vpdo, $filename, array $options = array()) {
        $this->vpdo =& $vpdo;
        ...
        if (!empty($this->_filename) && file_exists(dirname($this->_filename))) {
            if (file_exists($this->_filename)) {
                if ($this->getOption(vPDOZip::OVERWRITE, null, false) && is_writable($this->_filename)) {
                    if ($this->_archive->open($this->_filename, \ZipArchive::OVERWRITE) !== true) {
                        $this->vpdo->log(vPDO::LOG_LEVEL_ERROR, "xPDOZip: Error opening archive at {$this->_filename} for OVERWRITE");
                    }
                } else {
                    if ($this->_archive->open($this->_filename) !== true) {
                        $this->vpdo->log(vPDO::LOG_LEVEL_ERROR, "xPDOZip: Error opening archive at {$this->_filename}");
                    }
                }
            } elseif ($this->getOption(vPDOZip::CREATE, null, false) && is_writable(dirname($this->_filename))) {
                if ($this->_archive->open($this->_filename, \ZipArchive::CREATE) !== true) {
                    $this->vpdo->log(vPDO::LOG_LEVEL_ERROR, "xPDOZip: Could not create archive at {$this->_filename}");
                }
            } else {
                $this->vpdo->log(vPDO::LOG_LEVEL_ERROR, "xPDOZip: The location specified is not writable: {$this->_filename}");
            }
        } else {
            $this->vpdo->log(vPDO::LOG_LEVEL_ERROR, "xPDOZip: The location specified does not exist: {$this->_filename}");
        }
    }
    ...
    public function pack($source, array $options = array()) {
        $results = array();
        if ($this->_archive) {
            $target = $this->getOption(vPDOZip::ZIP_TARGET, $options, '');
            if (is_dir($source)) {
                    ...
                    while (($file = readdir($dh)) !== false) {
                        if ($this->checkExclude($target . $file, $options)) {
                            if (is_dir($source . $file)) {
                                if (($file !== '.') && ($file !== '..')) {
                                    $results = $results + $this->pack($source . $file . '/', array_merge($options, array(vPDOZip::ZIP_TARGET => $target . $file . '/')));
                                }
    ...
    protected function checkExclude($target, $options = array()) {
        ...
        if (empty($this->_exclusions)) {
            $exclude = $this->getOption(vPDOZip::EXCLUDE, $options, null);
            switch (true) {
    ...
    private function checkFiletype($filename, $allowedExtensions)
    {
        if (is_array($allowedExtensions)) {
            ...
            if (!in_array($ext, $allowedExtensions)) {
                $this->vpdo->log(vPDO::LOG_LEVEL_WARN, $filename .' can\'t be extracted, because the file type is not allowed.');
                return false;
            }
        }
        return true;
